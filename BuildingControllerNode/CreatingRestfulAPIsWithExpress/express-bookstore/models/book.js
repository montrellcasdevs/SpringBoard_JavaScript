...
class Book {
   ...
   *
   * data may include any of: {amazon_url, author, language, pages, publisher, title, year}
   *
  ...
  static async partialUpdate(isbn, data) {
    ...
    if (data.isbn) {
      throw { message: "You cannot change isbn", status: 400 };
    }

    const keys = Object.keys(data);
    if (keys.length === 0) {
      throw { message: "No data provided", status: 400 };
    }

    const validCols = ["amazon_url","author","language","pages","publisher","title","year"];
    ...
    for (let key of keys) {
      if (!validCols.includes(key)) {
        throw { message: `Invalid field: ${key}`, status: 400 };
      }
      cols.push(`${key}=$${idx}`);
    ...
    const querySql = `UPDATE books SET ${cols.join(", ")} WHERE isbn=$${idx} RETURNING isbn, amazon_url, author, language, pages, publisher, title, year`;
    values.push(isbn);
